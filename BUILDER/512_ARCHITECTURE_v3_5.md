...
**Rationale:** I6 imposes three distinct obligations. Evaluation-Unavailable DENY: when the gate cannot evaluate, the infrastructure-failure handler produces DENY — the commit boundary holds, the reason is disclosed, and retry is permitted. Transparent Denial: any constraint violation DENY must identify the failed invariant and disclose the governing rule. Human Default: on any adverse outcome, authority returns to the human party — exit, retry, and contest remain structurally available. Opaque denial, authority concentration at the gate, and permanent deprivation of human agency are all non-conformant regardless of intent. The authoritative elaboration is `512-core/KERNEL/I6_CONSTITUTIONAL_ELABORATION.md`.
...
| Field | Value |
|---|---|
| Document | `512_ARCHITECTURE_v3.5.md` |
| Version | 3.5 |
| Date | June 2026 |
...
- §8.4 Conformance table: gap record row updated — "after fail-open test" replaced with "after gate failure test / CVS sidecar gap record."
- §7.2 DORA: "fail-open behaviour / execution does not block" replaced with "Evaluation-Unavailable DENY / commit boundary holds / cause disclosed / retry permitted."
