...
This document is a technical specification and reference architecture for the Cryptographic Verification Sidecar (CVS). It is provided for informational, architectural, and implementation reference purposes only.

CVS records cryptographic evidence of observation. It does not certify correctness, compliance, safety, security, legality, or fitness for any purpose. No outcome is guaranteed.
...
**512 (constraint set):** 512 is treated in this work as a discovered constraint category — a set of invariants that a Commit Gate may satisfy. No license is asserted by the authors over the constraint set itself as an abstract constraint category.
...
The full license text must be included in the repository as LICENSE and applies to this document and associated specification materials within the CVS repository unless explicitly stated otherwise.
...
Evidence Objects are small — typically 1–5 KB each. At 1,000 events per second, daily storage (compressed) is approximately 52 GB. At 100 events per second, approximately 5 GB per day *(illustrative)**.
...
### 7.3 CVS Exhibits the Structural Properties of a Technical Risk Control

Independent evidence infrastructure is structurally consistent with a technical risk control when it exhibits three properties: it is external to the execution path (not an internal log); resistant to insider modification (not under the insured's sole control); and independently verifiable (requiring no trust in the insured's toolchain). CVS exhibits all three by architectural requirement — not by policy or procedure that could be altered without detection.

Whether any specific underwriter classifies CVS as a qualifying control, and what premium treatment results, is determined by that underwriter's criteria, not by this document. Organisations seeking insurance treatment for CVS deployment must engage their underwriter directly. No premium outcome is implied, projected, or guaranteed.
...
**Disclosure Logging.** Every disclosure request **MUST** be recorded as an Evidence Object, hashed, chained, and anchored, including requester identity, query parameters, timestamp, and outcome.
...
### 9.2 Behaviours Disqualifying Conformance
...
### 9.3 Claims Incompatible with CVS-Conforming Status

A deployment making any of the following claims is not accurately described as a CVS-conforming implementation:

- that it guarantees correctness of observed system behaviour or legitimacy of observed events;
- that it guarantees truth;
- that it prevents wrongdoing or fraud;
- that it enforces compliance with any legal or regulatory requirement;
- that it replaces legal, regulatory, or insurance judgment;
- that it guarantees complete system coverage without an anchored Declared Observation Surface explicitly filed;
- that it guarantees insurance premium reduction of any specific amount;
- that it guarantees regulatory approval or audit certification of any kind;
- that it provides autonomous compliance enforcement.

CVS records evidence only. All determination of correctness, sufficiency, and legal effect is external to the architecture.
...
**Conformance is binary.** All 16 tests must pass. A single failure is non-conformance. There is no partial conformance tier and no exception category. When a test fails: document the failure with precise reproduction steps; remediate the implementation; retest from the beginning of the checklist. Conformance is established by completing the full checklist from the beginning following any remediation.
...
| **Filename** | `CVS_ARCHITECTURE_v3.2.md` |
| **Version** | 3.2 |
...
|---|---|---|
| Added | §0 Normative Relationships | `VCP_AND_CVS.md` — parallel development acknowledgment and architectural differentiators |
| Added | §0 Normative Relationships | `CANONICAL_COMMITMENT.md` — permanent priority record across three independent proof layers |
| Modified | Document Control | Version 3.1 → 3.2; Publication Date April 2026 → May 2026; Status Pre-Seal Hardening → Active |
...
| Status | Section | Description |
|---|---|---|
| Modified | Document Control | Version 3.0 → 3.1; hardening pass April 2026 |
...
| Status | Section | Description |
|---|---|---|
| Added | Document header | Pre-hardening phase notice callout linking to repository `PRE_HARDENING_NOTICE.md` |
| Added | §0 Primitive Boundary | New subsection declaring CVS primitive scope: CVS defines evidence formation at the execution boundary; does not define system architecture, orchestration, deployment models, or implementation patterns; derivative responsibility rests entirely with the implementing party |
| Modified | Document Control | Version 2.9 → 3.0; Publication Date March 2026 → April 2026; Status field added: Pre-Seal Hardening |

**Additions:** Pre-hardening notice; §0 Primitive Boundary subsection.
...
| Status | Section | Description |
|---|---|---|
| Modified | §7.3 | Heading renamed from "CVS Satisfies the Structural Criteria for a Technical Risk Control" to "CVS Exhibits the Structural Properties of a Technical Risk Control" — removes regulatory implication framing; body rewritten: "qualifies as a technical risk control when it satisfies three properties" → "is structurally consistent with a technical risk control when it exhibits three properties"; "CVS satisfies all three" → "CVS exhibits all three"; "Organizations" → "Organisations" per British spelling convention |
| Modified | §9.2 | Heading renamed from "Prohibited Behaviors (MUST NOT)" to "Behaviours Disqualifying Conformance"; introductory sentence added: "A deployment exhibiting any of the following behaviours is not conforming" — converts enforcement heading to definitional framing; MUST NOT list retained as RFC 2119 technical specification language |
| Modified | §9.3 | Heading renamed from "Claims a Conformant Implementation Must Not Make" to "Claims Incompatible with CVS-Conforming Status"; "A conformant CVS implementation MUST NOT claim that it:" replaced with "A deployment making any of the following claims is not accurately described as a CVS-conforming implementation:"; list items recast as "that it…" statements; closing sentence added: "CVS records evidence only. All determination of correctness, sufficiency, and legal effect is external to the architecture." |
| Modified | §9.4 | Closing sentence "Full Conformance: All 16 tests pass. Partial conformance (1–2 failures) requires documented exceptions and remediation timelines. Three or more failures constitute non-conformance." replaced with binary conformance statement consistent with CVS_IMPLEMENTATION §16.4 — removes partial conformance tier and documented-exceptions category; conformance defined as binary with full checklist retest required after any remediation |
| Modified | §2.2 | "compliance dashboards" → "conformance dashboards" in Interpretation Plane description — ANTI_DRIFT §13 language alignment |
| Modified | §2.2 diagram | "Compliance" label in ASCII diagram → "Conformance" — ANTI_DRIFT §13 language alignment |
| Modified | §9.1 | "MUST be logged as an Evidence Object" → "MUST be recorded as an Evidence Object" in Disclosure Logging — ANTI_DRIFT §13: logging → evidence capture |
...
| Replaced | §-1 | Entire §-1 Legal Notice and Limitation of Liability (§-1.1 through §-1.13) replaced with consolidated Apache 2.0-aligned notice (§-1.1 through §-1.6); sections consolidated: No Warranty, No Reliance/Certification/Endorsement, Limitation of Liability, Licensing Posture (512 vs CVS), License |
| Removed | §-1.3 | No Professional Advice — content consolidated into §-1.1 Informational Nature |
| Removed | §-1.4 | No Guarantee of Coverage or Compliance — content consolidated into §-1.1 and §-1.3 |
| Removed | §-1.5 | Ownership and Licensing — Open Commons Declaration — replaced by §-1.5 Licensing Posture and §-1.6 License |
| Removed | §-1.6 | Public Ledger References — ledger-agnostic statement retained in §0 and §3.4 |
| Removed | §-1.7 | License (prior) — replaced by §-1.6 with Apache 2.0 repository-obligation language |
| Removed | §-1.8 | Jurisdictional Scope — consolidated into §-1.4 Limitation of Liability |
| Removed | §-1.9 | Financial Projection Disclaimer — consolidated into §-1.1 and §-1.3 |
| Removed | §-1.10 | No Reliance — consolidated into §-1.3 |
| Removed | §-1.11 | No Endorsement — consolidated into §-1.3 |
| Removed | §-1.12 | Independent Verification Requirement — consolidated into §-1.1 |
| Removed | §-1.13 | Builder Responsibility — consolidated into §-1.4 |
| Modified | §1.1, §3.1, §3.3, §6.5, §7.2 | Five `*(illustrative — see §-1.9)*` cross-references updated to `*(illustrative)*` following §-1.9 removal |
